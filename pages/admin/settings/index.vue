        ...
        <div class="container">
            <div class="settings__wrap">
                ...
                <div class="settings__form">
                    <div class="settings__form-title">
                        Цены в рублях за штуку
                    </div>
                    <div class="settings__label" v-for="item in prices" :key="item.id">
                        <div class="settings__text">
                            Формат: {{ item.name }}
                        </div>
                        <input class="settings__input" type="text" v-model="item.price">
                        <button class="settings__btn btn" @click="changeNumber(item)">
...
export default {
    ...
    methods: {
        async getAllPrices() {
            try {
                await this.$axios.get('api/prices').then(res => {
        ...
        async changeNumber(item) {
            ...
            try {
                const d = await this.$axios.put(`api/prices/${item.id}`, obj)
                if (d) {
                    this.getAllPrices()
                }
            } catch (error) {
    ...
    async mounted() {
        if (this.$route.name === 'admin-settings') {
            document.querySelector('body').style.background = '#FEFFD2'
        }
